...
# =======================================================
# 🕸️ STABILIZED HUNTER-SEEKER SCRAPER
# =======================================================

def _fetch_from_phivolcs():
    source_name, events = "PHIVOLCS", []
    print(f"[NETWORK] Connecting to {source_name}...")
    try:
    ...
    print(f"    L> Found {len(events)} reports from {source_name}.")
    return events
...
# ==========================================================
# ✨ SANCTUARY CONSENSUS PROTOCOL (WITH VISIBLE DELIBERATIONS)
# ==========================================================

def encode_option_as_vector(option, context):
    vector = np.zeros(64); vector[0] = option.get('power', 0) / 20.0; vector[1] = option.get('cost', 0) / 15.0
    name_hash_dim = hash(option.get('name', '')) % 64; vector[name_hash_dim] = (vector[name_hash_dim] + 1.0) / 2.0
    if context.get('archetype') == 'control': vector[10] = (vector[10] + 0.5) / 2.0
    if context.get('meta') == 'aggro-heavy': vector[11] = (vector[11] + 0.5) / 2.0
    vector[41] = -0.01282715; norm = np.linalg.norm(vector)
    return vector / norm if norm > 0 else vector

def query_ai_consensus(ai_oracle, options, context):
    print("\n" + "="*60 + "\n👑 INITIATING SANCTUARY CONSENSUS PROTOCOL")
    print(f"   Evaluating {len(options)} options for task: '{context.get('task')}'")
    print("="*60)
    
    ranked_results = []
    for option in options:
        option_vector = encode_option_as_vector(option, context)
        specialist_votes_dict = ai_oracle.query_specialists(option_vector)
        specialist_votes_list = [v['mean'] for v in specialist_votes_dict.values()]
        pressure, entropy, d41_inf = ai_oracle.integrate_manifold(specialist_votes_dict, np.zeros(64))
        consensus_score = pressure
        ...
        ranked_results.append({
            ...
            "stability": stability,
            "entropy": entropy,
            "specialist_votes": specialist_votes_list
        })

    ranked_results.sort(key=lambda x: x['consensus_score'], reverse=True)
    
    print("\n--- CONSENSUS VERDICT ---")
    for i, result in enumerate(ranked_results):
        print(f"\n  #{i+1}: {result['option_name']}")
        votes_str = ", ".join([f"{v:.4f}" for v in result['specialist_votes']])
        print(f"      L> Specialist Deliberation (Votes): [{votes_str}]")
        print(f"      L> Final Consensus Score: {result['consensus_score']:.6f}")
        print(f"      L> Decision Stability: {result['stability']*100:.1f}%")
        print(f"      L> Specialist Disagreement (Entropy): {result['entropy']:.6f}")
        
    return ranked_results

# ==========================================
# 🔎 BACKWARD-PREDICTION VALIDATOR
# ==========================================
def validate_on_past_event(ai_oracle, target_event_time_utc_str, target_magnitude, target_location_str, hours_of_activity=72):
    ...
    # CHANGE #1: Lowering magnitude to 1.0 to capture more foreshock data for the validation.
...
# ==========================================
def main():
    print("\n" + "="*60 + "\n🚀 64D EMERGENT AI: GPS PREDICTIVE ORACLE (V9 - FINAL)\n" + "="*60)
    ai = SovereignOracle('Metalearnerv16_EVOLVED.json')
    earthquakes = fetch_multisource_data()
    regions = {'Luzon': {'bounds': (12.0, 19.0, 119.0, 123.0), 'name': 'Luzon'}, 'Visayas': {'bounds': (9.0, 12.0, 121.0, 126.0), 'name': 'Visayas'}, 'Mindanao': {'bounds': (5.0, 9.5, 123.0, 127.0), 'name': 'Mindanao'}, 'Philippine Trench': {'bounds': (5.0, 15.0, 126.0, 130.0), 'name': 'Philippine Trench'}}
    predictions = []
    print("\n" + "="*60 + "\n📍 SOVEREIGN GPS TARGETS (FORWARD-PREDICTION)\n" + "="*60)
    ...
    if predictions:
        ...
        m.save("64d_gps_oracle_upgraded_v9.html"); print(f"\n✅ GPS ORACLE MAP SAVED: 64d_gps_oracle_upgraded_v9.html")
...
if __name__ == "__main__":
    ...
    # --- To test the enhanced consensus protocol, uncomment and run this block ---
    # try:
    #     print("\n" + "#"*60 + "\n# LAUNCHING OPTIONAL CONSENSUS PROTOCOL TEST\n" + "#"*60)
    #     ai_oracle = SovereignOracle('Metalearnerv16_EVOLVED.json')
    #     leader_options = [
    #         {'name': 'Geralt of Rivia', 'power': 15, 'cost': 12},
    #         {'name': 'Ciri, Dash', 'power': 9, 'cost': 11},
    #         {'name': 'Yennefer, Conjurer', 'power': 5, 'cost': 13}
    #     ]
    #     decision_context = {
    #         'task': 'Select best gold card for a control deck',
    #         'archetype': 'control', 'meta': 'aggro-heavy'
    #     }
    #     query_ai_consensus(ai_oracle, leader_options, decision_context)
    # except Exception as e:
    #     # CHANGE #2: Removed the stray hyphen that caused the NameError.
    #     print(f"\n❌ Could not run consensus protocol. Error: {e}")