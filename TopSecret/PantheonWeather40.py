...
def load_weather_memory():
    ...
    try:
        ...
        print(f"⚠️  Could not load weather memory: {e}")
        return {'history': []}
...
def save_weather_memory(memory):
    ...
    try:
        ...
        print(f"⚠️  Could not save weather memory: {e}")
...
def fetch_open_meteo_data(lat, lon):
    ...
    try:
        ...
        data = response.json()
        print("✅ Open-Meteo data acquired (with historical context).")
        return data
    except Exception as e:
...
def fetch_weatherapi_backup(lat, lon):
    ...
    print(f"🌐 Fetching backup data from WeatherAPI.com...")
    try:
        ...
        response = requests.get(url, params=params, timeout=20)
        response.raise_for_status()
        print("✅ WeatherAPI backup data acquired.")
        return response.json()
    except Exception as e:
        print(f"⚠️  WeatherAPI backup failed: {e}")
        return None
...
def merge_weather_sources(primary, backup):
    ...
    print("🔄 Cross-validating weather data from multiple sources...")
...
def load_pantheon():
    ...
    try:
        ...
        print(f"❌ AI Core Error: Could not load '{WEIGHT_FILE}'. {e}")
        return None
...
def run_weather_deliberation(nodes, atmospheric_vec, actual_precip_sum, actual_rain_prob, weather_pattern=None):
    ...
    if weather_pattern and weather_pattern['rain_days'] >= 2:
        memory_boost = 10
        final_predictions['ai_insights'].append(f"📊 Wet pattern detected: {weather_pattern['rain_days']} rainy days recently")
    
    pressure_change = manifold[9] * 10
    if pressure_change < -2:
        memory_boost += 15
        final_predictions['ai_insights'].append(f"📉 Pressure dropping: {pressure_change:.1f}mb in 6hrs")
...
def assess_lightning_risk(hourly_data, day_index):
    ...
    if risk_score >= 80:
        return "⚡ HIGH", "High", max_cape
    elif risk_score >= 50:
        return "⚡ MODERATE", "Moderate", max_cape
    elif risk_score >= 30:
        return "⚡ LOW", "Low", max_cape
    else:
        return "○ MINIMAL", "Minimal", max_cape
...
def display_forecast(location_name, raw_data, ai_predictions, weather_pattern):
    ...
    for i in range(7):
        ...
        if ai_rain_prob > 70 or total_rain_mm > 10:
            ...
            condition = "🌤️ Isolated Showers Possible"
        ...
        print(f"\n🗓️  {day_date.strftime('%A, %B %d')} {data_quality}")
        print("-" * 60)
        print(f"   Forecast: {condition}")
        print(f"   🌡️  High: {temp_high:.1f}°C | Low: {temp_low:.1f}°C (AI: {ai_pred['temp']:.1f}°C)")
        print(f"   💧  Rain: {ai_rain_prob:.0f}% | Expected: {total_rain_mm:.1f}mm ({rain_intensity})")
        
        if ai_boost > 5:
            print(f"       └─ AI Boost: +{ai_boost:.0f}% confidence adjustment")
        
        print(f"   ⏰  Rain Timing: {rain_timing}")
        ...
        print(f"   🍃  Wind: {wind_desc} ({wind_speed_kmh:.0f} km/h)")
        print(f"   🌅  Sun: {sunrise} - {sunset}")
        print(f"   🤖 AI Confidence: {conf_text} ({conf_val:.1f}%)")
    ...
    print("\n" + "=" * 75)
    print("💡 ENHANCED WITH AI WEATHER MEMORY & PRESSURE ANALYSIS")
...
# ============================================================================
def main():
    print("--- 🌋 PANTHEON AI WEATHER MODEL v4.0 (Phase 1 Enhanced) ---")
    print("    ✨ NEW: Weather Memory, Pressure Trends, CAPE Analysis\n")
    ...
    if weather_pattern:
        print(f"🧠 Loaded weather memory: {len(weather_memory.get('history', []))} days of history")
    ...
    for day_index in range(7):
        ...
        print(f"🧠 AI deliberating on {datetime.fromisoformat(day_str).strftime('%A, %b %d')}...")
    ...
    print("✅ AI deliberation complete.\n")
    ...
    try:
        ...
        print(f"⚠️  Could not update weather memory: {e}")
